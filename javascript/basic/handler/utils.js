...
class HandlerUtils {
     ...
     * @return {import('discord.js').TextChannel} Returns a TextChannel object.
     */
    getDefaultChannel (guild) {
        /** @type {import('discord.js').TextChannel} */
        // @ts-ignore
        let channel = guild.channels?.cache
                .filter(c => !c.isThread() && c.type === 'GUILD_TEXT' && c.permissionsFor(guild.me).has(['SEND_MESSAGES']), true)
                .first();

            if (!channel && guild.rulesChannel) channel = guild.rulesChannel;
            else if (channel) return channel;
            else return;
    }
     ...
     */
    async quickError (interaction, message) {
        try {
     ...
     * @param {string} message The error message.
     */
     async quickSuccess (interaction, message) {
        try {
